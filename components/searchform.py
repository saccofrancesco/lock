...
class SearchForm:
    ...
    def __init__(
        self, master: ctk.CTkFrame, criteria: str, top: ctk.CTkToplevel
    ) -> None:
        """
        ...
        self.criteria_entry: CustomEntry = CustomEntry(
            master, f"Enter {criteria.capitalize()} to search"
        )
        ...
        self.criteria_entry.pack(
            padx=10, pady=5, anchor="w", expand=True, fill=ctk.BOTH
        )
    ...
    def get_entry_values(self) -> tuple[str]:
        ...
        """
        return (entry.get() for entry in [self.master_entry.entry, self.criteria_entry])
    ...
    def update_search_button_state(self) -> None:
        ...
        if all(values):
            self.search_btn.configure(state="normal", fg_color=self.SEARCH_BGCOLOR)
        else:
            self.search_btn.configure(
                state="disabled", fg_color=self.DISABLED_SEARCH_BGCOLOR
            )
    ...
    def search_password(self) -> None:
        ...
        passwords_list: list = search_password(
            self.master_entry.entry.get(), self.criteria, self.criteria_entry.get()
        )
